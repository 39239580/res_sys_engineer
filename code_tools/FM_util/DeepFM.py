...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # 作为默认的计算图
            ...
            self.train_phase = tf.placeholder(tf.bool, name="train_phase")  # 默认为shape= None
            ...
            self.y_first_order = tf.reduce_sum(tf.multiply(self.y_first_order), 2)
            ...
            self.squared_sum_features_emb = tf.reduce_sum(self.summed_features_emb)  # None *k
            ...
            # init
            self.save = tf.train.Saver()  # 实例化对象，进行声明
    ...
    def _initialize_weights(self):
        ...
        weights["feature_embeddings"] = tf.Variable(
            tf.random_normal([self.feature_size, self.embedding_size], 0.0, 0.01),  # M*k, 均值为0，标准差为0.01正态分布
            name="feature_embeddings")   # feature_size *K

        weights["feature_bias"] = tf.Variable(
            tf.random_uniform([self.feature_size, 1], 0.0, 1.0),  # 生成均匀分布 M*1  的
        ...
        weights["bias_0"] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(1, self.deep_layers[0])), dtype=tf.float32)  # 1*layers[0]
        # 正态分布  偏置
    ...
    def batch_norm_layer(self, x, train_phase, scope_bn):   # BN 层
        bn_train = batch_norm(x, decay=self.batch_norm_decay, center=True, scale=True, updates_collections=None,
                              is_training=True, reuse=None, trainable=True, scope=scope_bn)   # 训练阶段
        bn_inderence = batch_norm(x, decay=self.batch_norm_decay, center=True, scale=True, updates_collections=None,
                                  is_training=False, reuse=None, trainable=True, scope=scope_bn)  # 预测阶段， 只有钱箱
